...
const byte rel[]   = {  16,  17,  18 /*,  19,  20,  21,  22,  23 */ }; // Relais Pins 16 heizl, 17 regen, 18 fan
...
// Zeit
const boolean setTime = false;  // Set Time 
byte second       = 00, 
     minute       = 36, 
     hour         = 22,
     weekdayVal   = 1,
...
const byte SMTP_PORT = 25; // Port 25 = default, secure = 587 
...
int minFillDay         = 1800; // Gesammtfassungsvermögen des Tanks
...
const byte loggCounterMax = 24; // Maximale Anzahl der schreibversuche auf SDKarte bevor logging disabled wird
...
// bytes
const byte atmVersion = 107; // Software Version
// Minutly Array Vars
...
byte    fillArrDSize = 0;
byte      minTempDay = 85;          // initialwert
byte      minWetDay  = 99;          // initialwert
byte initial = 1;
byte grund, feuchte, temperaturC, errCode, mini, maxi, steps, 
     bericht, sendCounter, loggCounter, counter, displayVar, 
     maxTempDay, maxWetDay, /* Counter Vars */ i, j;

// byte arrays
byte startDate[] = { 0, 0, 0, 0 }; 
...
// ints
int      maxFillDay = minFillDay;  // initialwert gleich Tankfassungsangabe minFillDay = 18l
unsigned int startZeit, laufZeit;
...
// Blitz
//byte C7[8] = { B00010, B00100, B01000, B00100, B00010, B10100, B11000, B11100 };
...
void setup() {
  ...
  Serial.println("--------------------------------------------------");
  Serial.print("  ATM Arduino Temperatur Mailerdaemon Version ");
  Serial.print(split(atmVersion,0));
  Serial.print(".");
  Serial.println(split(atmVersion,1));
  ...
  lcd.setCursor(0 ,1);
  lcd.print("  version ");
  lcd.print(split(atmVersion,0));
  lcd.print(".");
  lcd.print(split(atmVersion,1));
  lcd.print("   ");
  delay(2000);
...
void loop() { 
  ...
  if(initial == 1) {
    ...
    // Startzeit und Datum merken
    startZeit = join(hour,minute);
    startDate[0] += day;
    startDate[1] += month;
  ...
  if( currMillis - prevMillis > 1000 ){
    ...
    // Sensorwerte alle 15 Sek abfragen
    if( second == 45 || second == 30 || second == 15 || second == 0 ) {
    ...
    // Anzeige Daten LCD Screens
    if( displayVar < 2 ) {
    
      heart();        // Show flashing Heartsymbol
      
      if( !useLogg ) { lcd.setCursor(14,0); lcd.write(char(208)); clearer = true; } // SD-Karte
        else { clearer = false; } 

      // Ventilator Anzeige und Schaltung
      if( temperaturC > ( warnTemp - 3 ) ) { 
        lcd.setCursor(14,0); 
        lcd.write(1); 
        clearer = true; 
        relState[2] = HIGH;
      ...
      if( !relState[1] && ( ( hour == rain[2] && ( minute >= rain[0] && minute < rain[1] ) ) && 
          ( second >= 0 && second < rain[3] ) && liter != 0 &&
          ( weekdayVal == 2 || weekdayVal == 4 || weekdayVal == 6 || weekdayVal == 7 ) ) ) {
        lcd.setCursor(14,0); 
        lcd.write(6);
        clearer = true;
        relState[1] = HIGH;
        ...
        if( temperaturC < ( warnTemp - 3 ) ) clearer = false;
        relState[1] = LOW;
      ...
      if( hour >= 22 || hour < 6 ) {
        ...
        // Turn Backlight off
        if( backlight && ( ( hour == 22 && minute == 30 ) && second == 1 ) ) { 
        ...
        if( relState[0] ) { lcd.setCursor(12,0); lcd.write(8); } // Sonne
         else { lcd.setCursor(12,0); lcd.write(" "); }
        
        // Turn Backlight on
        if( !backlight && hour == 6 && minute == 30 && second == 1 ) { 
          digitalWrite(LCD_LIGHT_PIN, HIGH);
          backlight = true; 
        }
      }
    ...
    // Laufzeit hochzählen 
    if( split(laufZeit,1) > 59 ) laufZeit += 40; 
  ...
  if( logging && useLogg && !versenden ) {
    ...
    if( loggCounter < loggCounterMax ) {
      ...
      if (dataFile) {
        ...
        dataFile.print(second%10);
        dataFile.print(";");
        if(versenden) { dataFile.print( grund ); } else { dataFile.print("log"); }
        dataFile.print(";");
      ...
      // MaxLogCount reached
      Serial.println("Max Logg Counter reached, Logging disabled!");
  ...
  if( currMillisM - prevMillisM > 60000 ){
    ...
    if( liter > ( maxFillDay / 100 ) ) maxFillDay = liter * 100; // Maxwert merken
    if( liter < (minFillDay / 100) ) minFillDay = liter * 100; // Minwert merken
  ...
  if( currMillisH - prevMillisH > 3600000 ){
    ...
    useLogg = false; // reset faillogging
  ...
  if( currMillisD - prevMillisD > 86400000 ){
    ...
    if( fillArrDCounter >= fillArrDSize ){
      ...
      // Arrays mit Counter als index weiter befüllen
      fillArrD[fillArrDCounter]  = liter;
      // und danach Zähler einen weiter setzen
      fillArrDCounter++;
    } // Ende Füllstands Array befüllen
  ...
  if( ( ( millis() - updateTime ) > interval ) && !checked ) {
    ...
    if ( minute % logg == 0 /* && second == 0 */ ) {
      logging = true; // reset logging
    }
...
//  EthernetClient client;
    ...
    if (client.connect(mailServer, SMTP_PORT)) {
      ...
      if (client.connected()) {
        ...
        // Authentication Maybe you have to change
        ...
        client.println("<html>");
        client.println("<head>");
        client.println("<meta http-equiv='content-type' content='text/html; charset=ISO-8859-15'>");
        ...
        client.print("  .intext { text-align:left;border-top:1px solid #CCCCCC; ");
        ...
        client.println(colorized);
        client.println("; }");
        client.println("  .small { font-size:14pt; }");
        client.println("  #time {  }");
        client.println("  .subwrap { }");
        client.println("  .smooth { color:#999999; }");
        client.println("  .smoothie { color:#AAAAAA; font-size:12pt; }");
        client.println("  .smoothier { color:#BBBBBB; font-size:10pt; }");
        client.println("  .middle { clear:both; }");
        client.println("  .left { float:left; }");
        client.println("  .right { float:right; }");
        client.println("  .gchart { margin:20px 10%; width:80%; }");
        ...
        client.print(liter);
        client.print("</b> Liter mit Wasser gef&uuml;llt. ");

        client.print("Vom Boden aus sind das <b>");
        client.print(fuellstand);
        client.print("cm</b>. Der Sensorabstand zur Wasseroberfl&auml;che betr&auml;gt <b>");
        client.print( fill(triggerPin,echoPin) );
        client.print("cm</b>");
        
        
        if( liter <= reFill && liter > (minFillDay / 100) ) {
          client.print(" und es sollte langsam ans Wasser Nachf&uuml;llen gedacht werden!");
        } else {
          client.print(". ");
        }
        
        if( (minFillDay / 100) > 0  && grund != 5 ){
          client.println("<br /><br />\n\n");
          client.print("Der niedrigste erreichte F&uuml;llstand lag bei <b>");
          client.print( (minFillDay / 100) );
        ...
        if( grundArrCounter >= (sizeof(grundArr)/sizeof(byte)) ) {
          ...
          if( grundMeldung != 10 && grundTempWarn != 10 && grundTempAlarm != 10 && grundFeuchteWarn != 10 && grundFeuchteAlarm != 10 && grundFuellstandWarn != 10 && grundFuellstandAlarm != 10 ) {
            ...
            client.print("&chma=10,120,5,5");
            
            client.print("&chl=");
            if( grundFuellstandAlarm != 0 ) { client.print("F%C3%BCllAlarm"); }
            if( grundFuellstandWarn != 0 ) { if( grundFuellstandAlarm != 0 ) { client.print("|"); } client.print("F%C3%BCllWarn"); }
            if( grundFeuchteAlarm != 0 ) { if( grundFuellstandWarn != 0 ) { client.print("|"); } client.print("FeuchteAlarm"); }
            if( grundFeuchteWarn != 0 ) { if( grundFeuchteAlarm != 0 ) { client.print("|"); } client.print("FeuchteWarn"); }
            if( grundTempAlarm != 0 ) { if( grundFeuchteWarn != 0 ) { client.print("|"); } client.print("TempAlarm"); }
            if( grundTempWarn != 0 ) { if( grundTempAlarm != 0 ) { client.print("|"); } client.print("TempWarn"); }
            if( grundMeldung != 0 ) { if( grundTempWarn != 0 ) { client.print("|"); } client.print("Meldung"); }
                        
                     
            client.print("&chdl=");
            if( grundFuellstandAlarm != 0 ) { client.print("F%C3%BCllstandAlarm"); }
            if( grundFuellstandWarn != 0 ) { if( grundFuellstandAlarm != 0 ) { client.print("|"); } client.print("F%C3%BCllstandWarn"); }
            if( grundFeuchteAlarm != 0 ) { if( grundFuellstandWarn != 0 ) { client.print("|"); } client.print("FeuchteAlarm"); }
            if( grundFeuchteWarn != 0 ) { if( grundFeuchteAlarm != 0 ) { client.print("|"); } client.print("FeuchteWarn"); }
            if( grundTempAlarm != 0 ) { if( grundFeuchteWarn != 0 ) { client.print("|"); } client.print("TempAlarm"); }
            if( grundTempWarn != 0 ) { if( grundTempAlarm != 0 ) { client.print("|"); } client.print("TempWarn"); }
            ...
            client.print("&chco=");
            if( grundFuellstandAlarm != 0 ) { client.print("8080FF"); }
            if( grundFuellstandWarn != 0 ) { if( grundFuellstandAlarm != 0 ) { client.print("|"); } client.print("0000FF"); }
            if( grundFeuchteAlarm != 0 ) { if( grundFuellstandWarn != 0 ) { client.print("|"); } client.print("004ECB"); }
            if( grundFeuchteWarn != 0 ) { if( grundFeuchteAlarm != 0 ) { client.print("|"); } client.print("008FCB"); }
            if( grundTempAlarm != 0 ) { if( grundFeuchteWarn != 0 ) { client.print("|"); } client.print("FF0000"); }
            if( grundTempWarn != 0 ) { if( grundTempAlarm != 0 ) { client.print("|"); } client.print("FF7300"); }
            if( grundMeldung != 0 ) { if( grundTempWarn != 0 ) { client.print("|"); } client.print("00800F"); }
            ...
            if( grundFeuchteAlarm != 0 ) { if( grundFuellstandWarn != 0 ) { client.print(","); } client.print(grundFeuchteAlarm); }
            if( grundFeuchteWarn != 0 ) { if( grundFeuchteAlarm != 0 ) { client.print(","); } client.print(grundFeuchteWarn); }
            if( grundTempAlarm != 0 ) { if( grundFeuchteWarn != 0 ) { client.print(","); } client.print(grundTempAlarm); }
            if( grundTempWarn != 0 ) { if( grundTempAlarm != 0 ) { client.print(","); } client.print(grundTempWarn); }
            if( grundMeldung != 0 ) { if( grundTempWarn != 0 ) { client.print(","); } client.print(grundMeldung); }
        ...
        if( grund != 5 ) {
          ...
          client.print(split(startZeit,0));
          client.print(":");
          client.print(split(startZeit,1)/10);
          client.print(split(startZeit,1)%10);
          client.println("</b> Uhr gestartet");
          if( grund != 5 ) {
            
            client.print(" und l&auml;uft jetzt seit <b>");
            
            if( split(laufZeit,0) > 24 ) {
              client.print( split(laufZeit,0) / 24 );
              client.print("</b> Tagen<b>");
              
              if( ( split(laufZeit,0) ) - ( split(laufZeit,0) / 24 * 24 ) > 0 ) {
                client.print(", ");
                client.print( ( split(laufZeit,0) ) - ( split(laufZeit,0) / 24 * 24 )  );
                client.print("</b> Stunden und <b>");
              } else {
                client.print("</b> und <b>");
              }
              
            } else {
              
              if( split(laufZeit,0) > 0 ) {
                client.print( ( split(laufZeit,0) ) );
                client.print("</b> Stunden und <b>");
              }
              
            }
            
            if( split(laufZeit,1) > 9 ) {
              client.print(split(laufZeit,1)/10);
              client.print(split(laufZeit,1)%10);
            } else {
              client.print(split(laufZeit,1));
            }
            
            client.print("</b> Minuten");
        ...
        client.print("Der RAM verbrauch liegt aktuell bei <b>");
        client.print( availableMemory() );
        client.println("</b> byte freiem Speicher und <b>");
        client.print( 8192 - availableMemory() );
        client.print("</b> byte verwendeten Speicher.");
        
        client.println("<br /><br />\n\n");
                
        // geschaltete RelaisPins als kleine Übersicht
        for(i = 0; i < (sizeof(rel)/sizeof(byte)); i++) {
          if( relState[i] == HIGH && i == 0 ) client.println("<b>&Uuml;bersicht der geschalteten Relais</b><br/><br/>");
          if( relState[i] == HIGH ) {
            client.print("Relais <b>#");
            client.print(i);
            client.print("</b> ist Eingeschaltet ");
            client.println("<br/>");
            if( i == (sizeof(rel)/sizeof(byte)) ) client.println("<br/><!-- ende relais -->");
          }
        }
        ...
        client.print("<div id='time'>Diese Mail wurde ");
        ...
        client.println("<div class='subwrap'>");
        
        // kostet zuviel RAM
        
//        client.print("ATM by <a href='mailto:");
//        client.print(benutzer);
//        client.print("@");
//        client.print(domain);
//        client.print("'>");
//        client.print(benutzer);
//        client.println("</a> 2013 <br />");
        
//        client.print("<span class='smooth'>");
//        client.print("Local IP ");
//        client.print(Ethernet.localIP());
//        client.println("</span><br />");
        
//        client.print("<span class='smoothie'>");
//        client.print("Absendegrund: ");
//        client.print(grund);
//        //client.print(grundStr( grund, 1 ));
//        client.println("</span><br />");
        
        client.println("<span class='smoothier'>");
        client.print("version ");
        client.print(split(atmVersion,0));
        client.print(".");
        client.print(split(atmVersion,1));
        client.println("</span>");
        
        client.println("</div>"); // ENDE subwrap
...
int split( int wert, int returnVar ){ 
  ...
  if( wert < 9999 || returnVar > 1 ){
    int einer, zehner, hunderter, tausender;
    tausender = wert / 1000 * 1000;
    ...
    if( returnVar == 0 ) { // returns default the first
      return ( ( tausender / 1000 * 10 ) + (hunderter / 100)   );
    } else {
      return (zehner + einer);
    }
  } else {
...
void showStat(){
  ...
  Serial.print("%|");
  Serial.print(fill(triggerPin,echoPin));
  Serial.print("cm|");
//  Serial.print( ( mapfloat(fill(triggerPin,echoPin),0,42,42,0) - 5 ) );
//  Serial.print("cm|");
  Serial.print(liter);
  Serial.print("l");
  Serial.print("  ");     // Spacer
  ...
  Serial.print("  ");     // Spacer
  Serial.print(split(laufZeit,0)/10);
  Serial.print(split(laufZeit,0)%10);
  Serial.print(":");
  Serial.print(split(laufZeit,1)/10);
  Serial.print(split(laufZeit,1)%10);
  Serial.print("  ");     // Spacer
  Serial.print(availableMemory());
  Serial.print("/");
...
void showStatLCD(){
  ...
  if( displayVar == 1 ){
    ...
    lcd.print("D");
    if(tempArrDCounter < 10) lcd.print(" ");
    ...
    lcd.setCursor(0 ,1);
    // Tage
    if( split(laufZeit,0) > 23 ) { 
      if( ( split(laufZeit,0) / 24 ) < 100 ) lcd.print(" ");
      if( ( split(laufZeit,0) / 24 ) < 10 ) lcd.print(" ");
      lcd.print( split(laufZeit,0) / 24 );
      lcd.print(" ");
      // Stunden wenn mindestens 1 Tag
      lcd.setCursor(4 ,1);
      if( ( ( split(laufZeit,0) )  - ( ( split(laufZeit,0) / 24 ) * 24 ) ) < 10 ){
        lcd.print(" ");
      } 
      lcd.print( ( split(laufZeit,0) ) - ( ( split(laufZeit,0) / 24 ) * 24 ) );
      
    } else { 
      // Kein Tag laufzeit, nur Stunden und Minuten anzeigen
      ...
      // Stunden
      if( ( split(laufZeit,0) ) < 10 ){
        lcd.print(" ");
      } 
      lcd.print( split(laufZeit,0) );
  ...
  if( displayVar == 4 ){
    ...
    if(second > 30) {
      ...
      lcd.setCursor(0 ,1);
      lcd.print("min");
      if( ( (minFillDay/100) ) < 10 ) lcd.print(" ");
      lcd.print( ( minFillDay / 100.0 ) );
      lcd.setCursor(7 ,1); // 
      lcd.print("l");
      
      lcd.setCursor(8 ,1);
      lcd.print("max");
      if( ( maxFillDay / 100 ) < 10 ) lcd.print(" ");
      lcd.print( ( maxFillDay / 100.0 ) );
      lcd.setCursor(15 ,1);
      lcd.print("l ");
...
//      if( WERT == 1 ) lcd.print("Mo ");
//      if( WERT == 2 ) lcd.print("Di ");
//      if( WERT == 3 ) lcd.print("Mi ");
//      if( WERT == 4 ) lcd.print("Do ");
//      if( WERT == 5 ) lcd.print("Fr ");
//      if( WERT == 6 ) lcd.print("Sa ");
//      if( WERT == 7 ) lcd.print("So");
    } else {
  ...
  if( displayVar < 0 ) displayVar = 0;
  if( displayVar > 7 ) displayVar = 0;
